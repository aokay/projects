...
shinyServer(function(input, output, session) {
  ...
  output$bikesharePlot <- renderPlot({
    ...
    isolate(input$date)
    isolate(bike_year_data<-train_full[grep(input$date,train_full$time),])
    day_hour_counts <- as.data.frame(aggregate(bike_year_data[,"count"], list(bike_year_data$day, bike_year_data$time$hour), mean))
    isolate(day_hour_counts)
    
      l<-labs(title=
          if(input$goButton2 >0) { 
          paste(input$date,sep="")
          } 
      )
    
    }
    
    if(input$goButton1==0 && input$goButton2 == 0) {
      l<-labs(title="All Years, All Months, All days")
    }
